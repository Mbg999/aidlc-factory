...
- **stage internals + output schema**: see [`stage/workspace-scout.md`](.claude/agents/stage/workspace-scout.md).
...
3. **Unit Decomposer (conditional)** — skip when ANY of:
   ...
   When skipping due to ComplexityGov, follow Step 4.5 skip enforcement. Otherwise run the shared spawn loop. Stage internals: [`stage/unit-decomposer.md`](.claude/agents/stage/unit-decomposer.md).
