...
description: AIDLC Orchestrator's parallel-safety layer. Owns the file-glob lock registry and Python AST symbol-drift detection. Detects conflicts when multiple parallel subagents touch overlapping paths or change shared interfaces. NOT a spawnable subagent — orchestrator invokes aidlc-scripts/factory_conflict.py directly.
model: inherit
readonly: true
is_background: false
---
...
> **Architectural note:** the Conflict Resolver is **not** a spawnable
...
2. User decides:
   ...
   - **Manual merge** — user resolves outside the orchestrator, then runs
     `/orchestrator factory-build <run-id>` again to continue.
...
3. Parallel spawn — delegate to N subagents in one message (N ≤ 4):
     "Use code-generator for unit-1 at <path-1>, unit-2 at <path-2>, ..."
