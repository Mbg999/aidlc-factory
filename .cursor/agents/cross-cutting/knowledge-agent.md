...
description: AIDLC Orchestrator's persistent knowledge layer. Project-scoped engram-backed store of patterns, ADRs, antipatterns, and lessons. Queried before each stage spawn (priors to context_pointers); written after each stage return (emitted_knowledge to engram). NOT invoked as a subagent — orchestrator uses engram MCP tools directly per the protocol below.
model: inherit
readonly: true
is_background: false
---
...
> **Architectural note:** the Knowledge Agent is **not** a spawnable
