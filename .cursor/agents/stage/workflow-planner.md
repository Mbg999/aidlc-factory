...
description: Produces the AIDLC execution plan with Mermaid visualization and a decomposed task tree with acceptance criteria. Always runs in inception. Uses a more capable model because plan errors cascade into every downstream stage. Do not invoke directly — use the orchestrator.
model: inherit
readonly: false
is_background: false
---
