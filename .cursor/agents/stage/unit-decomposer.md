...
description: Decomposes the approved execution plan into per-unit specs. Conditional — runs only when the plan explicitly enumerates 2 or more units OR requirements call out distinct services/components. Do not invoke directly — use the orchestrator.
model: inherit
readonly: false
is_background: false
---
