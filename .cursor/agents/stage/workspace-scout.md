...
description: Detects greenfield vs brownfield workspace state, identifies tech stack, decides next AIDLC phase. First stage of every AIDLC inception run. Spawned by the AIDLC orchestrator with a path to its input handoff YAML. Do not invoke directly — use the orchestrator.
model: inherit
readonly: false
is_background: false
---
