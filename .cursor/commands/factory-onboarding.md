...
Use `/factory-spec "<request>"`. A dedicated orchestrator agent delegates
...
3. **Delegates workspace-scout** to learn your codebase
4. **Delegates requirements-analyst** to write a spec (may ask you questions)
...
This is the heavy lifter. It delegates **code-generator** × N units in parallel
...
- **Budget gates** — pre-flight check before each delegation (ok / downshift / skip / halt)
...
Fans out 4 reviewers in parallel (all `is_background: true`):
| Reviewer | Focus |
|----------|-------|
...
This runs the full pipeline against `aidlc-scripts/`, `.cursor/agents/`, and
