...
Spawn any agent from `.opencode/agents/custom/` via the standard spawn loop
...
`factory_conflict.py` — lock registry + AST drift detection. Full spec: [`cross-cutting/conflict-resolver.md`](.opencode/agents/cross-cutting/conflict-resolver.md).
...
Engram-backed persistent memory (MCP, NOT Task()). Full spec: [`cross-cutting/knowledge-agent.md`](.opencode/agents/cross-cutting/knowledge-agent.md).
...
- **stage internals + two-pass output schemas**: see [`stage/requirements-analyst.md`](stage/requirements-analyst.md).
...
3. If any AST drift conflict was written, surface BEFORE the approval gate (user decides per [`cross-cutting/conflict-resolver.md`](.opencode/agents/cross-cutting/conflict-resolver.md)).
...
Stage internals (sub_stage transitions, plan/generated/approved schemas): [`stage/code-generator.md`](stage/code-generator.md).
...
Stage internals: [`stage/build-test-agent.md`](stage/build-test-agent.md).
...
All four share `reviewer.input.v1.json` and `reviewer.output.v1.json`. Stage internals: [`stage/reviewer-code.md`](stage/reviewer-code.md) and siblings.
