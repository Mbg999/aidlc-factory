...
- **Deferred to Phase 5.5 (follow-up):** TS/JS AST diff via tree-sitter + tree-sitter-typescript. Will extract `export function`/`export class`/**`export interface`**/**`export type`** for `.ts/.tsx/.js/.jsx` files — interfaces and types are the high-value catch since they're what break TS consumers silently. Deep type-system drift (generics narrowing, conditional types) stays out of scope; that needs `tsc --noEmit` or LSP integration (Phase 7+). User decision 2026-05-08: ship Phase 5 as Python-only, add TS later when a real TS conflict goes uncaught.
