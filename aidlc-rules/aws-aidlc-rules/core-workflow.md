...
- `/factory-onboarding [command]` — guided tour of the orchestrator system
...
- `/factory-plan <run-id>` — decompose plan into per-unit specs (multi-component features only)
- `/factory-build <run-id>` — layer-parallel code generation with file-glob locks + AST symbol drift checks
...
Roles, contracts, budgets, and parallelism rules: see
`.claude/agents/orchestrator.md` or `.opencode/agents/orchestrator.md` (for
OpenCode users), `.aidlc-orchestrator/contracts/`, and
