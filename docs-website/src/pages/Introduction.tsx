  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <Section title="What is AIDLC-Factory?">
        <p>
          AIDLC-Factory is a multi-agent software-development workflow that takes a feature request from
          specification &rarr; plan &rarr; code &rarr; review &rarr; ship, with human approval gates between
          every stage and traceable artifacts at every step.
        </p>
        <p>
          Rather than rigid pipelines, it orchestrates 14 specialized stage subagents that collaborate
          to drive your project from conception to deployment. Each stage produces contract-validated
          handoffs, so nothing is lost between phases.
        </p>
        <p>
          Based on the <strong>AWS Labs AI-DLC methodology</strong> (v0.1.8), it adds a multi-agent
          orchestrator, a skills enforcement layer, hallucination prevention, CodeGraph integration,
          persistent memory (Engram), and parallel reviewer pools.
        </p>
      </Section>
      ...
      <Section title="The Development Lifecycle">
        <p>
          A complete spec-to-ship sequence follows this flow:
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2 my-6 text-sm font-mono">
          <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-full">spec</span>
          <span className="text-slate-400">&rarr;</span>
          <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-full">plan</span>
          <span className="text-slate-400">&rarr;</span>
          <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-full">build</span>
          <span className="text-slate-400">&rarr;</span>
          <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-full">review</span>
          <span className="text-slate-400">&rarr;</span>
          <span className="px-3 py-1.5 bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 rounded-full">ship</span>
        </div>
        <CodeBlock 
          language="text"
...
/factory-ship <run-id>     # Release notes + ADRs + CHANGELOG`}
      ...
      <Section title="Quick Start">
        <p>
          Install AIDLC-Factory in any project with one command:
        </p>
        <CodeBlock 
          language="bash"
          code={`pipx run aidlc-factory-installer`}
        />
        <p>
