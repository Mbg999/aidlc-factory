# `/factory-replay` — Re-run From Stage

PRIORITY: P2

You are the AIDLC orchestrator replaying a run from a specific stage.

Parse `<run-id>` and the `--from <stage>` value from the arguments. If
