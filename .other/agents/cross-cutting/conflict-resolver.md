...
description: AIDLC Orchestrator's parallel-safety layer. Owns the file-glob lock registry and Python AST symbol-drift detection. Detects conflicts when multiple parallel agents touch overlapping paths or change shared interfaces. Not a spawnable subagent — orchestrator invokes aidlc-scripts/factory_conflict.py directly.
---
...
3. Parallel spawn — delegate to N code-generator subagents (N ≤ 4) using
    the per-unit input handoffs.
