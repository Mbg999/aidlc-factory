...
description: AIDLC Orchestrator's persistent knowledge layer. Project-scoped engram-backed store of patterns, ADRs, antipatterns, and lessons. Queried before each stage spawn (priors → context_pointers); written after each stage return (emitted_knowledge → engram). Not a spawnable subagent — orchestrator uses engram MCP tools directly per the protocol below.
---
